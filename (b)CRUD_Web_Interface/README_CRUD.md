...
This folder contains the PHP/MySQL CRUD web application we developed to visualize, manage, and export results from the ICSR assessment pipeline.  
It allows us to easily **browse**, **edit**, **import/export**, and **analyze** the cases and performance metrics generated by the automation workflows.
...
> 📝 *Tip:* You can rename or remove unused helper files once the core CRUD pages are working.
...
| **random_cases.php** | Selects random case IDs for testing or sampling. |
...
|----------|---------------|
| Blank page | Enable PHP errors in the file with: `ini_set('display_errors', 1);` |
| DB connection fails | Check `db.php` credentials and make sure MySQL is running |
| Import doesn’t work | Ensure the Excel headers match the table columns |
| Empty export | You might have active filters — clear them and try again |

---

## 🧠 Example Workflow

1. Log in → go to the dashboard (`index.php`)  
2. Import new cases via `import.php`  
3. Review or edit them in the table view  
4. Export the latest results to Excel  
5. Analyze agreement or model performance downstream  
